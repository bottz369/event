...
def draw_text_mixed(draw, xy, text, primary_font, fallback_font, fill):
    ...
    Args:
        draw: ImageDrawオブジェクト (Noneの場合は計測のみ)
        xy: 描画開始位置 (x, y)
        text: 描画する文字列
        primary_font: ユーザーが選択したメインフォント
        fallback_font: 日本語表示用の補助フォント
        fill: 文字色
    
    Returns:
        (total_w, max_h): 描画した文字列全体の幅と高さ
    """
    x, y = xy
    total_w = 0
    max_h = 0
    ...
    for char in text:
        ...
        # ★ここが重要: ユーザー選択フォントで表示できない場合のみ切り替わる
...
def draw_text_with_shadow(base_img, text, x, y, font_path, font_size_px, max_width, fill_color, 
    ...
    while can_resize and current_size > min_size:
        # draw_text_mixed を使って幅を計算（文字ごとにフォント判定される）
