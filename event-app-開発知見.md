...
streamlit 1.59.2 と fastapi 0.115.6 は共存可能。クリーン解決で starlette==0.41.3 となり、fastapi 0.115.6 は
DEFAULT_EXCLUDED_CONTENT_TYPES を参照しないため import エラーは起きない(streamlit は tornado ベースで starlette 非依存)。
以前の本番クラッシュ(root に fastapi を足した d1926ad → revert 59535a0)は「無指定/新しめの fastapi が新しい starlette を要求
→ 別要因で古い starlette が入る」不一致が原因で、fastapi==0.115.6 の明示ピンで解消。
→ Docker は root+bot 両方を install する full install 方針で問題なし。
...
- ✅ **bot ドキュメント整合 完了(2026-07-16、94f36ad / 135dc54)**: bot/requirements.txt 先頭コメント +
  bot/README.md(ローカル起動節・Railway 節)を「Docker は root+bot を full install / streamlit と
  fastapi は共存可能」の実態に整合(旧「streamlit 共存不可 / pandas 除外」記述を撤去)。
  Railway の Custom Start Command 空運用(罠36)も明記。挙動不変・コメント/ドキュメントのみ。
