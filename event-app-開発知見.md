...
- ⏳ **次アクション = 本番グループでの LINE 実機テスト**(起動 → メンション → ボタン → 画像 → 2 枚受信 →
  オーナー退会で停止 → 再招待 → 再起動)。
  reply token の 1 分制限を超えないか(画像受信フローは DL + 更新 + 生成で最長)をそこで確認する。
  **§48(21:38)/ §49(22:33)は push 済み。§50(B-4)のみ未 push**。
  §50 を push すると、既存グループも一度「起動」しないと使えなくなる点に注意
  (静的許可リストが外れ、Storage の起動ストアが空から始まるため)。
