...
## 28. Phase 5 grid スライスB2(セレクタ)+ スライスC(font)= grid ビュー完了(2026-07-08)

✅ grid ビューの db.query 全滅を達成。B1(§26)に続き B2・C を実施し、grid.py から
view→service→repo の3層規律違反(DB 直叩き)を完全排除。本番反映済み(origin/main = 6640afe)、
実機テスト全項目合格。main 直コミット。

### 28.1 スライスB2: セレクタを list_projects_for_selector へ(commit 1d6f70a)
- grid.py L70-77 の db.query(TimetableProject).all() + インライン label 生成 + selectbox を
  project_service.list_projects_for_selector()([(id,label)]・@st.cache_data)へ移行。
- id 逆引きは format_func 方式を採用({label:id} マップ再構築は不採用)。理由: 日付なしプロジェクトは
  全て "---- {title}" に寄るため、同 title だと label 完全重複が起こりうる。マップ方式は dict キー
  衝突で誤 id を引くが、options に id 実体を持たせる format_func 方式なら label 重複でも選択の
  同一性が壊れない。未選択は options 先頭 None → format_func で "(選択)"、if selected_id: が None を弾く
  既存構造をそのまま活用。key= は付けない(選択永続は ws_active_project_id 側が担保、罠18 回避)。
- import から TimetableProject トークン撤去。
- 意図的差分(G5・合意済み): 日付なしプロジェクトのラベルが "None {title}"(旧・f"{None}" の
  リテラル化)→ "---- {title}"(共有窓口の '----' 補完)に変化。改善方向。日付ありは完全一致。
  並び順は双方 event_date or "0000-00-00", reverse=True で一致。
- 検証: scratch/verify_selector_id_parity.py で id 逆引き parity。本命 = label 完全重複
  (日付なし+同 title)ケースで旧 {label:id} は len(map)<len(pairs) で id 欠落=衝突、新 format_func は
  id 実体保持で両方引ける、を対比 assert。(1)-(4) ALL PASS。

### 28.2 スライスC: Font read の service 化 + L61 db 撤去(commit 5932700 inert / 6640afe 置換)
- grid.py の Font 系 db.query 群(get_sorted_font_list / create_font_specimen_img / grid ローカルの
  check_and_download_font)を font_service 経由に移し、最後に L61 db=next(get_db()) + finally close を撤去。
- 【共用 helper 無改造 + service ラッパ】get_sorted_font_list / create_font_specimen_img は
  【4ビュー共用】(grid/flyer/assets/timetable)のため utils の定義・シグネチャを一切触らない。
  font_service が own_db(SessionLocal 所有)を helper に渡す薄いラッパ(list_sorted_fonts / build_specimen)を
  提供。他3ビューは無改修で従来どおり動く(git diff 空で証明済み)。
- check_and_download_font(grid 専用)は font_service.ensure_font_available(filename) へロジック移設し
  grid から撤去。戻り値4状態(cached / downloaded_url / downloaded_db / not_found)。
- 【toast は view 戻し】check_and_download_font 内の st.toast を service に持ち込まず、service は状態を
  戻り値で返し、grid.py(view)が戻り値で toast を出す。理由: services 層の画面非依存
  (streamlit を import しない)を保つ=将来 API/LINE Bot 化の前提(§11.3)。font_service に
  streamlit import 無し(grep 0件で証明)。
- 新設2ファイル: repositories/font_repo.py(Asset/AssetFile の純 read 2窓口・commit しない)、
  services/font_service.py(SessionLocal 所有・helper ラッパ + ensure_font_available)。
  ensure_font_available は check_and_download_font の分岐順を厳密踏襲
  (空ガード→makedirs→cached 早期 return→URL経路→binary経路→not_found、URL 失敗時は binary へ fall-through)。
- 未使用 import 整理(requests / get_db / Asset / AssetFile / get_image_url / utils 2シンボル撤去、
  IMAGE_DIR / FONT_DIR は他所使用で温存)。
- 検証: scratch/verify_font_service_parity.py。(A)実 font_service を fake 依存でロードし list_sorted_fonts/
  build_specimen が own_db 素通し・戻り素通し・close することを assert。(B)ensure_font_available の
  DL 分岐が旧 check_and_download_font と同一到達(8ケース、fall-through 系(3)(4)(7)含む)を assert。
  ALL PASS。共用 helper 無改造(utils/__init__.py git diff 空)・他ビュー無変更(flyer/assets/timetable
  git diff 空)を証明。

### 28.3 grid ビュー完了の総括
- grid.py の db.query = 0件(全滅)。view から DB 直叩きを排除、view→service→repo の3層規律に完全準拠。
- Phase 5 の残りは flyer ビューのみ(flyer_json 動的キー30+・罠18/22 的構造が論点)。

### 罠32(パターン): 4ビュー共用 helper は「シグネチャ据え置き + service ラッパ」で1ビューだけ移行できる
共用 helper(get_sorted_font_list 等)を1ビューの service 化都合で db 引数を外すと他ビューが壊れる。
→ helper は無改造のまま、service が own_db を所有して helper に渡す薄いラッパを噛ませると、
対象ビューだけ DB 直叩きを排除でき他ビューは無波及。git diff 空(共用 helper・他ビュー)で波及ゼロを証明する。
