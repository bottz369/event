...
→ グリッド/フライヤーの並び順で loser 名が参照され、該当アーティストの表示落ち・
  名前不一致リスク。**⑤-b で対応予定**(⑤-a は TimetableRow のみのスコープで合意)。
...
- **Phase 5(残りビュー移行)**: artists **完了**。
  残り = **⑤-b**(grid_order_json の名寄せ・data_json 残留・loser 画像孤児化・
  `views/artists.py` の未使用 DB セッション撤去)、**grid ビュー**、**flyer ビュー**。
- ⑤-b 着手時は main から新ブランチを切る。
