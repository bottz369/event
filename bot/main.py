...
def handle_event(event: dict, config: BotConfig) -> None:
    ...
    if msg_type == "text":
        ...
        # --- 段階C C-1 / C-1.1 / C-1.2: 記入テンプレのやりとり ---
        #
        # ★判定順は「概要が入っているか」が先。これは C-1.2 で正式化した
        #   【ワンショット投入】の仕様:
        #     - メンション + 概要(■見出し等が 2 つ以上)  → 種別ボタンを出さず直接解析
        #     - 「新規作成」と概要が同じメッセージにある    → 概要があるので直接解析
        #     - 「新規作成」だけ(概要なし)              → 種別ボタンを出す
        #   テンプレを配らずに手元の告知文をそのまま投げられるようにするため。
        #   素の「新規作成」は見出しを含まないので、この分岐には掛からない。
        #   種別は本文の【イベント種別】行から取り、無ければ LLM 推定に委ねる
        #   (判定できなければエコーにその旨が出る。対話での確認は C-2)。
