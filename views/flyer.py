...
def is_glyph_available(font, char):
    ...
    try:
        ...
        # cmapがない場合はTrueとみなすが、一般的にTrueTypeなら持っているはず
        return True
    except Exception:
        return True
...
def draw_text_with_shadow(base_img, text, x, y, font, font_size_px, max_width, fill_color, 
    ...
    # 1. フォールバック用フォントの準備
    fallback_font = font # デフォルトはメインフォント
    ...
    if fallback_font_name:
        try:
            ...
            if os.path.exists(fb_path):
                ...
                print(f"Fallback font not found: {fallback_font_name}")
        except Exception as e:
...
def create_flyer_image_shadow(
    ...
    ticket_info_list,
    common_notes_list,
    system_fallback_font=None # 標準フォントファイル名
):
    # 1. 背景の読み込みとA4化
...
def render_flyer_editor(project_id):
    ...
    with c_conf:
        with st.expander("🖼️ 基本設定", expanded=True):
            ...
            # 日付フォーマット
            date_fmt_opts = ["EN (例: 2025.2.15.SUN)", "JP (例: 2025年2月15日 (日))"]
            ...
            st.radio("📅 日付表示形式", date_fmt_opts, key="flyer_date_format_radio")
        ...
        with st.expander("📐 コンテンツ・余白調整", expanded=False):
            st.markdown("**メイン画像（グリッド/TT）の最大サイズ**")
            c_sz1, c_sz2 = st.columns(2)
            with c_sz1: st.slider("横幅 (%)", 50, 100, step=1, key="flyer_content_scale_w")
            with c_sz2: st.slider("高さ (%)", 50, 100, step=1, key="flyer_content_scale_h")
            
            st.markdown("---")
            st.markdown("**チケット情報の行間**")
    ...
    with c_prev:
        ...
        if st.button("画像を生成する", type="primary", use_container_width=True):
            ...
            args = {
                ...
                "system_fallback_font": sys_fallback # 追加
            }
