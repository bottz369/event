...
def load_image_from_url(url):
    try:
        ...
        # 画像DLエラーはここで握りつぶさずNoneを返して呼び出し元でログする
        return None
...
def draw_one_row(draw, canvas, base_x, base_y, row_data, font_path):
    ...
    # ---------------------------------------------------------
    # [レイヤー1] アーティスト写真描画
    # ★重要: import database をここで行うことで循環参照を回避
    # ---------------------------------------------------------
    if name_str and name_str not in ["OPEN / START", "開演前物販", "終演後物販"]:
        try:
            # ★ここでインポートします（ファイルの先頭には書きません）
                    ...
                    if artist.image_filename:
                        ...
                        if url:
                            ...
                            if img:
                                ...
                                # print(f"✅ Paste OK: {name_str}") # ログ用
                            else:
                                draw_debug_info(draw, f"DL Error", base_x + 10, base_y + 10, "red")
                        else:
                            draw_debug_info(draw, "URL Gen Fail", base_x + 10, base_y + 10, "red")
                    else:
                        draw_debug_info(draw, "No Image", base_x + 10, base_y + 10, "yellow")
                else:
                    draw_debug_info(draw, f"Not Found", base_x + 10, base_y + 10, "magenta")
            finally:
                db.close()
        except Exception as e:
            draw_debug_info(draw, f"SysErr: {e}", base_x + 10, base_y + 10, "red")
