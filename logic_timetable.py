...
def draw_one_row(draw, canvas, base_x, base_y, row_data, font_path):
    ...
    # ---------------------------------------------------------
    # [レイヤー1] アーティスト写真描画
    # ★重要: import database をここで行うことで循環参照を完全回避
    # ---------------------------------------------------------
                ...
                if artist:
                    if artist.image_filename:
                        ...
                        if url:
                            ...
                            draw_debug_info(draw, "URL Error", base_x + 10, base_y + 10, "red")
                    else:
                        draw_debug_info(draw, "No Image", base_x + 10, base_y + 10, "yellow")
                else:
                    draw_debug_info(draw, "Not Found", base_x + 10, base_y + 10, "magenta")
            finally:
                db.close()
        except Exception as e:
            draw_debug_info(draw, f"Err: {e}", base_x + 10, base_y + 10, "red")
