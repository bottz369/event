...
_SYSTEM_PROMPT = (
    ...
    "括弧より前を「名前」と「金額」に分け、括弧の中が備考。"
    "名前は行頭の語(Sチケット / Aチケット / 当日 など)、"
    "残り(¥ や「各+」を含む部分)がすべて金額。"
    ...
    "- ★チケットの金額は【書かれているとおりの文字列】にする。数値に直さない。"
    "通貨記号・桁区切り・「各+」のような接頭辞も落とさない"
    "(「¥6,000」→ \"¥6,000\"、「各+¥1,000」→ \"各+¥1,000\")。"
    "読み取れなければ null。\n"
    "- チケットの備考は【括弧の中だけ】。括弧が無ければ null。"
    "金額の一部(「各+」など)を備考に移さない。\n"
        ...
        "tickets": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    # ★金額は数値に潰さず「告知文の表記のまま」持つ。
                    #   Web のチケット欄(tickets_json.price)は text_input で
                    #   "¥6,000" / "各+¥1,000" のような表示文字列を入れる欄なので、
                    #   int 化すると通貨記号・桁区切り・「各+」が失われる。
                    "price": {"type": ["string", "null"],
                              "description": "告知文の表記のまま(例 ¥6,000 / 各+¥1,000)"},
...
def format_intake_echo(parsed: dict) -> str:
    ...
    if tickets:
        for t in tickets:
            # 金額は告知文の表記のまま(「¥6,000」「各+¥1,000」)なので単位を足さない
            price = t.get("price") or "(金額未記入)"
